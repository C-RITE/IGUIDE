...
int CDynDialogEx::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  if (m_pFont == NULL) {
    ...
    wcsncpy(LogFont.lfFaceName, _T("MS Sans Serif"), LF_FACESIZE);
    LogFont.lfHeight = 8;
...
INT_PTR CDynDialogEx::DoModal() 
{
  ...
  int nBufferSize =  sizeof(DLGTEMPLATE) + (2 * sizeof(WORD)) /*menu and class*/ + sizeof(m_strCaption) /*size of caption*/
    + sizeof(WORD) /*fontsize*/ + sizeof(LogFont.lfFaceName) /*size of fontname*/;
  ...
  if (hLocal != NULL) {
    ...
    *(WORD*)(pdest + 1) = 0;							// use default window class -- WORD to say it is 0 bytes
    pdest += sizeof(WORD);								// Increment
    memcpy(pdest, m_strCaption, sizeof(m_strCaption));			// Caption
    pdest += sizeof(m_strCaption);

    *(WORD*)pdest = m_wFontSize;						// font size
    pdest += sizeof(WORD);
    memcpy(pdest, LogFont.lfFaceName, sizeof(LogFont.lfFaceName));		// font name
    pdest += sizeof(LogFont.lfFaceName);
