...
void CPropertyGrid::DrawItem(CDC& dc, int w, int x, int y, vector<CItem>::iterator& it)
{
  ...
  else
  {
    ...
    case IT_FOLDER:
      {
        TCHAR szBuffer[1024];
        wcsncpy(szBuffer, strValue.c_str(), 1024);
...
LRESULT CPropertyGrid::OnEditChanged(WPARAM wParam, LPARAM lParam)
{
  ...
  if (pItem)
  {
    if (pItem->m_type == IT_STRING)
    {
      pItem->m_strValue = wchar_t((char*)wParam);
      pItem->m_undefined = false;
      GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    ...
    {
      if (pItem->m_pCustom->OnItemEdited(((wchar_t*)wParam)))
        GetOwner()->SendMessage(WM_PG_ITEMCHANGED, pItem->m_id);
      DeleteEditControl();
      Invalidate();
    }
...
void CPropertyGrid::EditFocusedItem()
{
  ...
  if (pItem)
  {
    ...
    else if (pItem->m_type == IT_DATETIME)
    {
      ...
      dlg.AddDlgControl("SysDateTimePick32", L"", STYLE_DATETIMEPICKER|DTS_SHORTDATEFORMAT, EXSTYLE_DATETIMEPICKER, CRect(7, 13, 60, 26), (void*) &dtValueDate);
      dlg.AddDlgControl("SysDateTimePick32", L"", STYLE_DATETIMEPICKER|DTS_TIMEFORMAT , EXSTYLE_DATETIMEPICKER, CRect(67, 13, 120, 26), (void*) &dtValueTime);
